...
#define __LL_RCC_CALC_PCLK2_FREQ(__HCLKFREQ__, __APB2PRESCALER__) ((__HCLKFREQ__) >> APBPrescTable[(__APB2PRESCALER__) >>  RCC_CFGR_PPRE2_Pos])
	...
	__STATIC_INLINE void LL_RCC_SetSAIClockSource(uint32_t SAIxSource) {
		MODIFY_REG(RCC->DCKCFGR, (SAIxSource & 0xFFFF0000U),
				(SAIxSource << 16U));
	}
#endif /* SAI1 */
  ...
  MODIFY_REG(RCC->DCKCFGR, RCC_DCKCFGR_PLLDIVR, PLLDIVR);
	...
	__STATIC_INLINE void LL_RCC_PLLI2S_ConfigDomain_SAI(uint32_t Source,
			uint32_t PLLM, uint32_t PLLN, uint32_t PLLQ_R, uint32_t PLLDIVQ_R) {
		__IO uint32_t *pReg =
...
#endif /* RCC_PLLI2SCFGR_PLLI2SM */
		MODIFY_REG(RCC->PLLI2SCFGR, RCC_PLLI2SCFGR_PLLI2SN,
  ...
  MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLM, PLLM);
		...
		MODIFY_REG(RCC->PLLI2SCFGR,
				RCC_PLLI2SCFGR_PLLI2SN | RCC_PLLI2SCFGR_PLLI2SP,
				PLLN << RCC_PLLI2SCFGR_PLLI2SN_Pos | PLLP);
	}
#endif /* SPDIFRX */
  ...
  return (uint32_t)(pllsrc | plli2sssrc0 | plli2sssrc1);
	...
	__STATIC_INLINE void LL_RCC_PLLSAI_ConfigDomain_SAI(uint32_t Source,
			uint32_t PLLM, uint32_t PLLN, uint32_t PLLQ, uint32_t PLLDIVQ) {
		MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLSRC, Source);
#if defined(RCC_PLLSAICFGR_PLLSAIM)
  ...
  MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLM, PLLM);
		...
		MODIFY_REG(RCC->PLLSAICFGR,
				...
				PLLN << RCC_PLLSAICFGR_PLLSAIN_Pos | PLLQ);
  ...
  MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLM, PLLM);
		...
		MODIFY_REG(RCC->PLLSAICFGR,
				...
				PLLN << RCC_PLLSAICFGR_PLLSAIN_Pos | PLLP);
	}
#endif /* RCC_PLLSAICFGR_PLLSAIP */
  ...
  return (uint32_t)(READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLM));
	...
	__STATIC_INLINE uint32_t LL_RCC_PLLSAI_GetN(void) {
		return (uint32_t) (READ_BIT(RCC->PLLSAICFGR, RCC_PLLSAICFGR_PLLSAIN)
