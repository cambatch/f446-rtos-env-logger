...
HAL_StatusTypeDef HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart,
	...
	if (huart->gState == HAL_UART_STATE_READY) {
		...
		if (HAL_DMA_Start_IT(huart->hdmatx, *(const uint32_t*) tmp,
				(uint32_t) &huart->Instance->DR, Size) != HAL_OK) {
...
HAL_StatusTypeDef UART_Start_Receive_DMA(UART_HandleTypeDef *huart,
	...
	tmp = (uint32_t*) &pData;
	if (HAL_DMA_Start_IT(huart->hdmarx, (uint32_t) &huart->Instance->DR,
			*(uint32_t*) tmp, Size) != HAL_OK) {
		/* Set error code to DMA */
		huart->ErrorCode = HAL_UART_ERROR_DMA;
