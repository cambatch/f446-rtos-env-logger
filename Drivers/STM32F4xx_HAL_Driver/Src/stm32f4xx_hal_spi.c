  ...
  __IO uint32_t tmpreg = 0U;
    ...
    if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
    {
      ...
      UNUSED(tmpreg);
		...
		/* Check the end of the transaction */
		if (SPI_EndRxTransaction(hspi, SPI_DEFAULT_TIMEOUT, tickstart)
				!= HAL_OK) {
			hspi->ErrorCode = HAL_SPI_ERROR_FLAG;
		}
...
static HAL_StatusTypeDef SPI_EndRxTxTransaction(SPI_HandleTypeDef *hspi,
	...
	/* Wait until TXE flag */
	if (SPI_WaitFlagStateUntilTimeout(hspi, SPI_FLAG_TXE, SET, Timeout,
			Tickstart) != HAL_OK) {
		SET_BIT(hspi->ErrorCode, HAL_SPI_ERROR_FLAG);
		return HAL_TIMEOUT;
	}
	...
	/* Timeout in us */
	count = SPI_BSY_FLAG_WORKAROUND_TIMEOUT
