...
HAL_StatusTypeDef HAL_I2C_Master_Receive(I2C_HandleTypeDef *hi2c,
	...
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		/* Send Slave Address */
		if (I2C_MasterRequestRead(hi2c, DevAddress, Timeout, tickstart)
				!= HAL_OK) {
			return HAL_ERROR;
		}
		...
		if (hi2c->XferSize > 0U) {
			if (hi2c->hdmatx != NULL) {
				...
				dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmatx,
						...
						(uint32_t) &hi2c->Instance->DR, hi2c->XferSize);
			} else {
				/* Update I2C state */
				hi2c->State = HAL_I2C_STATE_READY;
				hi2c->Mode = HAL_I2C_MODE_NONE;
...
HAL_StatusTypeDef HAL_I2C_Slave_Transmit_DMA(I2C_HandleTypeDef *hi2c,
	...
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		if (hi2c->hdmatx != NULL) {
			...
			/* Enable the DMA stream */
			dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmatx,
					(uint32_t) hi2c->pBuffPtr, (uint32_t) &hi2c->Instance->DR,
					hi2c->XferSize);
		} else {
			/* Update I2C state */
...
HAL_StatusTypeDef HAL_I2C_Slave_Receive_DMA(I2C_HandleTypeDef *hi2c,
	...
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		if (hi2c->hdmarx != NULL) {
			...
			/* Enable the DMA stream */
			dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmarx,
					(uint32_t) &hi2c->Instance->DR, (uint32_t) hi2c->pBuffPtr,
					hi2c->XferSize);
		} else {
			/* Update I2C state */
			hi2c->State = HAL_I2C_STATE_LISTEN;
...
HAL_StatusTypeDef HAL_I2C_Mem_Write_DMA(I2C_HandleTypeDef *hi2c,
	...
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		if (hi2c->XferSize > 0U) {
			...
			if (dmaxferstatus == HAL_OK) {
				/* Send Slave Address and Memory Address */
				if (I2C_RequestMemoryWrite(hi2c, DevAddress, MemAddress,
						MemAddSize, I2C_TIMEOUT_FLAG, tickstart) != HAL_OK) {
...
HAL_StatusTypeDef HAL_I2C_Mem_Read_DMA(I2C_HandleTypeDef *hi2c,
	...
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		if (hi2c->XferSize > 0U) {
			...
			if (dmaxferstatus == HAL_OK) {
				/* Send Slave Address and Memory Address */
				if (I2C_RequestMemoryRead(hi2c, DevAddress, MemAddress,
						MemAddSize, I2C_TIMEOUT_FLAG, tickstart) != HAL_OK) {
					/* Abort the ongoing DMA */
...
HAL_StatusTypeDef HAL_I2C_Master_Abort_IT(I2C_HandleTypeDef *hi2c,
	...
	if ((__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_BUSY) != RESET)
			&& ((CurrentMode == HAL_I2C_MODE_MASTER)
...
void HAL_I2C_EV_IRQHandler(I2C_HandleTypeDef *hi2c) {
	...
	if ((CurrentMode == HAL_I2C_MODE_MASTER)
		...
		if ((I2C_CHECK_FLAG(sr1itflags, I2C_FLAG_SB) == RESET)
				&& (IS_I2C_TRANSFER_OTHER_OPTIONS_REQUEST(CurrentXferOptions)
						== 1U)) {
			return;
		}
...
static void I2C_MemoryTransmit_TXE_BTF(I2C_HandleTypeDef *hi2c) {
	...
	if (hi2c->EventCount == 0U) {
		...
		if (CurrentState == HAL_I2C_STATE_BUSY_RX) {
			...
			hi2c->EventCount++;
		} else if ((hi2c->XferCount > 0U)
				&& (CurrentState == HAL_I2C_STATE_BUSY_TX)) {
			/* Write data to DR */
			hi2c->Instance->DR = *hi2c->pBuffPtr;
...
static void I2C_MasterReceive_RXNE(I2C_HandleTypeDef *hi2c) {
	if (hi2c->State == HAL_I2C_STATE_BUSY_RX) {
		...
		} else if ((hi2c->XferOptions != I2C_FIRST_AND_NEXT_FRAME)
				&& ((tmp == 1U) || (tmp == 0U))) {
	...
	if (hi2c->XferCount == 4U) {
		...
		*hi2c->pBuffPtr = (uint8_t) hi2c->Instance->DR;
		...
		if ((CurrentXferOptions != I2C_NEXT_FRAME)
				&& (CurrentXferOptions != I2C_FIRST_AND_NEXT_FRAME)) {
			/* Disable Acknowledge */
			CLEAR_BIT(hi2c->Instance->CR1, I2C_CR1_ACK);
		}
		...
		*hi2c->pBuffPtr = (uint8_t) hi2c->Instance->DR;
		...
		if ((CurrentXferOptions == I2C_FIRST_FRAME)
				|| (CurrentXferOptions == I2C_LAST_FRAME_NO_STOP)) {
			/* Disable Acknowledge */
			CLEAR_BIT(hi2c->Instance->CR1, I2C_CR1_ACK);
		} else if ((CurrentXferOptions == I2C_NEXT_FRAME)
				|| (CurrentXferOptions == I2C_FIRST_AND_NEXT_FRAME)) {
			/* Enable Acknowledge */
			SET_BIT(hi2c->Instance->CR1, I2C_CR1_ACK);
		} else {
	...
	if (hi2c->State == HAL_I2C_STATE_BUSY_RX) {
		if ((hi2c->EventCount == 0U) && (CurrentMode == HAL_I2C_MODE_MEM)) {
			...
			if (hi2c->XferCount == 0U) {
				...
				/* Prepare next transfer or stop current transfer */
				else if ((CurrentXferOptions != I2C_FIRST_AND_LAST_FRAME)
						&& (CurrentXferOptions != I2C_LAST_FRAME)
...
static void I2C_Slave_STOPF(I2C_HandleTypeDef *hi2c) {
	...
	if ((hi2c->Instance->CR2 & I2C_CR2_DMAEN) == I2C_CR2_DMAEN) {
		if ((CurrentState == HAL_I2C_STATE_BUSY_RX)
				|| (CurrentState == HAL_I2C_STATE_BUSY_RX_LISTEN)) {
			hi2c->XferCount =
					(uint16_t) (I2C_GET_DMA_REMAIN_DATA(hi2c->hdmarx));
			...
			hi2c->XferCount =
					(uint16_t) (I2C_GET_DMA_REMAIN_DATA(hi2c->hdmatx));
...
static void I2C_ITError(I2C_HandleTypeDef *hi2c) {
	...
	if (((uint32_t) CurrentState & (uint32_t) HAL_I2C_STATE_LISTEN)
			== (uint32_t) HAL_I2C_STATE_LISTEN) {
...
static void I2C_DMAXferCplt(DMA_HandleTypeDef *hdma) {
	...
	if ((((uint32_t) CurrentState & (uint32_t) HAL_I2C_STATE_BUSY_TX)
			== (uint32_t) HAL_I2C_STATE_BUSY_TX)
			|| ((((uint32_t) CurrentState & (uint32_t) HAL_I2C_STATE_BUSY_RX)
	...
	/* Check current Mode, in case of treatment DMA handler have been preempted by a prior interrupt */
		...
		if ((CurrentXferOptions == I2C_NO_OPTION_FRAME)
				...
				|| (CurrentXferOptions == I2C_OTHER_AND_LAST_FRAME)
				|| (CurrentXferOptions == I2C_LAST_FRAME)) {
			/* Generate Stop */
			SET_BIT(hi2c->Instance->CR1, I2C_CR1_STOP);
		}
