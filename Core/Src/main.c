  ...
  /* USER CODE BEGIN StartLcdTask */
	...
	for (;;) {
		if (xQueueReceive(xSensorToLcd, &sensorData, portMAX_DELAY) == pdPASS) {
			...
			// accel
			snprintf(line4, sizeof(line4), "Ax:%4ld Ay:%4ld Az:%4ld", sensorData.accelX, sensorData.accelY, sensorData.accelZ);

			// gyro
			int32_t gxDps = sensorData.gyroX / 1000;
			int32_t gyDps = sensorData.gyroY / 1000;
			int32_t gzDps = sensorData.gyroZ / 1000;
			snprintf(line5, sizeof(line5), "Gx:%ld Gy:%ld Gz:%ld", (long)gxDps, (long)gyDps, (long)gzDps);
  ...
  /* USER CODE BEGIN StartSensorTask */
	...
	for (;;) {
		...
		if(MPU6050_ReadRaw(&imuRaw)) {
			MPU6050_Convert(&imuRaw, MPU6050_ACCEL_FS_2G, MPU6050_GYRO_FS_250DPS, &imuScaled);
