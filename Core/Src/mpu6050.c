...
bool MPU6050_Init(MPU6050_AccelFs_t accelFs, MPU6050_GyroFs_t gyroFs) {
	...
	uint8_t who;
	if (read_reg(MPU6050_REG_WHO_AM_I, &who) != HAL_OK) {
		return false;
	}
	...
	// exit sleep, use pll
	if (write_reg(MPU6050_REG_PWR_MGMT_1, 0x01) != HAL_OK) {
		return false;
	}

	// sample rate
	if (write_reg(MPU6050_REG_SMPLRT_DIV, 9) != HAL_OK) {
		return false;
	}

	// DLPF
	if (write_reg(MPU6050_REG_CONFIG, 0x03) != HAL_OK) {
		return false;
	}

	// full scale gyro
	if (write_reg(MPU6050_REG_GYRO_CONFIG, (uint8_t) (gyroFs << 3))
			!= HAL_OK) {
		return false;
	}

	// full scale accel
	if (write_reg(MPU6050_REG_ACCEL_CONFIG, (uint8_t) (accelFs << 3))
			!= HAL_OK) {
		return false;
	}
...
bool MPU6050_ReadRaw(MPU6050_Raw_t *out) {
	...
	if (read_multi_reg(MPU6050_REG_ACCEL_XOUT_H, buf, sizeof(buf))
			!= HAL_OK) {
		return false;
	}
