...
static uint8_t MPU6050_WriteReg(uint8_t reg, uint8_t value) {
	return HAL_I2C_Mem_Write(&hi2c2, MPU6050_ADDR, reg, I2C_MEMADD_SIZE_8BIT, &value, 1, 50);
}

static uint8_t MPU6050_ReadReg(uint8_t reg, uint8_t *buf) {
	return HAL_I2C_Mem_Read(&hi2c2, MPU6050_ADDR, reg, I2C_MEMADD_SIZE_8BIT, buf, 1, 50);
}

static uint8_t MPU6050_ReadMulti(uint8_t reg, uint8_t *buf, uint16_t len) {
	return HAL_I2C_Mem_Read(&hi2c2, MPU6050_ADDR, reg, I2C_MEMADD_SIZE_8BIT, buf, len, 50);
}
...
bool MPU6050_Init(MPU6050_AccelFs_t accelFs, MPU6050_GyroFs_t gyroFs) {
	...
	// full scale gyro
	if(MPU6050_WriteReg(MPU6050_REG_GYRO_CONFIG, (uint8_t)(gyroFs << 3)) != HAL_OK) {
		return false;
	}

	// full scale accel
	if(MPU6050_WriteReg(MPU6050_REG_ACCEL_CONFIG, (uint8_t)(accelFs << 3)) != HAL_OK) {
		return false;
	}
...
bool MPU6050_ReadRaw(MPU6050_Raw_t *out) {
	...
	if(MPU6050_ReadMulti(MPU6050_REG_ACCEL_XOUT_H, buf, sizeof(buf)) != HAL_OK) {
		return false;
	}
...
void MPU6050_Convert(const MPU6050_Raw_t *raw, MPU6050_AccelFs_t accelFs, MPU6050_GyroFs_t gyroFs, MPU6050_Scaled_t *out) {
	int32_t accelLsbPerG = 16384; // for ±2g
	int32_t gyroLsbPerDps = 131;  // for ±250 dps

	 switch (accelFs) {
	    case MPU6050_ACCEL_FS_2G:  accelLsbPerG = 16384; break;
	    case MPU6050_ACCEL_FS_4G:  accelLsbPerG = 8192;  break;
	    case MPU6050_ACCEL_FS_8G:  accelLsbPerG = 4096;  break;
	    case MPU6050_ACCEL_FS_16G: accelLsbPerG = 2048;  break;
	 }

	switch (gyroFs) {
		case MPU6050_GYRO_FS_250DPS:  gyroLsbPerDps = 131;  break;
		case MPU6050_GYRO_FS_500DPS:  gyroLsbPerDps = 65;   break;
		case MPU6050_GYRO_FS_1000DPS: gyroLsbPerDps = 32;   break;
		case MPU6050_GYRO_FS_2000DPS: gyroLsbPerDps = 16;   break;
	}
